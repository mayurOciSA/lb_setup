resource "oci_core_instance" "lb_bkend_hs1" {
  ...
  display_name        = "bkend_hs1_${count.index}"
  ...
  create_vnic_details {
    ...
    hostname_label            = "hs1-bk-${count.index}"
  }
...
output "backend_instance_private_ip_hs1" {
  value = one(oci_core_instance.lb_bkend_hs1[*].private_ip)
}

output "backend_instance_public_ip_hs1" {
  value = one(oci_core_instance.lb_bkend_hs1[*].public_ip)
}
...
resource "null_resource" "execute_ansible_playbook_hs1" {
  count               = var.backend_hs1_count
  depends_on = [time_sleep.wait_minutes_hs1]
  ...
  provisioner "remote-exec" {
    ...
    connection {
      host        = oci_core_instance.lb_bkend_hs1[count.index].public_ip
      type        = "ssh"
      user        = "opc"
  ...
  provisioner "local-exec" {
    command = "sleep 20; ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -u opc -i '${oci_core_instance.lb_bkend_hs1[count.index].public_ip},' --private-key ${var.ssh_private_key_local_path} ./ansible/appdeploy.yml -v"
  }
}
